...
function BackToLibraryButton({ onClick }: { onClick: () => void }) {
  return (
    <div onClick={onClick} className="absolute content-stretch flex items-center justify-center left-[142px] p-[10px] top-[410px] cursor-pointer hover:opacity-80 transition-opacity" data-name="Back to Library button">
...
export default function OverlayUploadedPage({ onClose, fileName = '' }: { onClose: () => void; fileName?: string }) {
  return (
    <div className="relative w-[469px] h-[800px]" data-name="Overlay - Uploaded Page">
      {/* Clickable background to close */}
      <div className="absolute inset-0" onClick={onClose} />
      
      {/* Main overlay content - stop propagation to prevent closing when clicking inside white boxes */}
      <div className="absolute inset-0 pointer-events-none">
        {/* Top white box */}
        <div className="-translate-x-1/2 -translate-y-1/2 absolute bg-white h-[561px] left-[calc(50%-0.5px)] rounded-tl-[20px] rounded-tr-[20px] top-[calc(50%-115.5px)] w-[469px] pointer-events-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Bottom white box (rotated) */}
        <div className="-translate-x-1/2 -translate-y-1/2 absolute flex h-[116px] items-center justify-center left-[calc(50%-1.5px)] top-[calc(50%+223px)] w-[469px] pointer-events-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Green box (rotated) */}
        <div className="-translate-x-1/2 -translate-y-1/2 absolute flex h-[99px] items-center justify-center left-[calc(50%-1.5px)] top-[calc(50%+222.5px)] w-[453px] pointer-events-auto" onClick={(e) => e.stopPropagation()}>
          <div className="flex-none rotate-180">
            <div className="bg-[#d3f4ea] h-[99px] rounded-[12px] w-[453px]" />
          </div>
        </div>
        
        {/* Uploaded PLY file row */}
        <div className="pointer-events-auto" onClick={(e) => e.stopPropagation()}>
          <UploadedPlyFile fileName={fileName} />
        </div>
        
        {/* Black divider bar */}
        <div className="absolute bg-black h-[8px] left-px top-[553px] w-[469px] pointer-events-auto" onClick={(e) => e.stopPropagation()} />
